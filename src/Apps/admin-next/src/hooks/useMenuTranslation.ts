...
interface TranslatedMenuChildItem extends Omit<MenuChildItem, "multi_menu"> {
  childtitle?: string;
  multi_menu?: TranslatedMultiMenuItem[];
}

interface TranslatedMultiMenuItem extends MultiMenuItem {
  multiTitle?: string;
}

interface TranslatedMegaMenuItem extends MegaMenuItem {
  megamenutitle?: string;
  singleMegamenu?: TranslatedSingleMegaMenuItem[];
}

interface TranslatedSingleMegaMenuItem extends SingleMegaMenuItem {
  m_childtitle?: string;
}

interface TranslatedMenuItem extends Omit<MenuItem, "child" | "megamenu"> {
  title?: string;
  child?: TranslatedMenuChildItem[];
  megamenu?: TranslatedMegaMenuItem[];
}
...
const useMenuTranslation = (): UseMenuTranslationReturn => {
  ...
  const translateMenuItems = (items: MenuItem[]): TranslatedMenuItem[] => {
    return items.map((item) => {
      const translated: TranslatedMenuItem = { ...item };
      ...
      if (item.child) {
        translated.child = item.child.map((child) => {
          const translatedChild: TranslatedMenuChildItem = { ...child };
          if (child.childtitle) {
          ...
          if (child.multi_menu) {
            translatedChild.multi_menu = child.multi_menu.map((multi) => {
              const translatedMulti: TranslatedMultiMenuItem = { ...multi };
              if (multi.multiTitle) {
      ...
      if (item.megamenu) {
        translated.megamenu = item.megamenu.map((mega) => {
          const translatedMega: TranslatedMegaMenuItem = { ...mega };
          ...
          if (mega.singleMegamenu) {
            translatedMega.singleMegamenu = mega.singleMegamenu.map((single) => {
              const translatedSingle: TranslatedSingleMegaMenuItem = { ...single };
  ...
  const translatedMenuItems = translateMenuItems(menuItems as MenuItem[]);
  const translatedTopMenu = translateMenuItems(topMenu as MenuItem[]);
